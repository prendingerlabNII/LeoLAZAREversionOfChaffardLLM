...
class Llama(nn.Module):
    ...
    def forward(self, x, itr):
        ...
        diff = x[0,-1,0]-x[-1,-1,-1]
        if diff > 0 :
            trend = 'upward'
        elif diff <= 0 :
            trend = 'downward'
            
        prompt = f'The Electricity Transformer Temperature (ETT) indicates the electric power long-term deployment. Each data point consists of the target oil temperature and 6 power load features. Below is the information about the input time series:' \
        f'[Domain]: We usually observe that electricity consumption peaks at noon, with a significant increase in transformer load'\
        f'[Instruction]: Predict the next {self.pred_len} steps given the previous {self.seq_len} steps information attached'\
        f'[Statistics]: The input has a minimum of {min_val}, a maximum of {max_val}, and a mean of {mean_val}. The overall trend is {trend}.'
